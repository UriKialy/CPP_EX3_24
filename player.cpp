...
namespace ariel
{
    ...
    bool player::hasResources(const vector<string> &required_resources) const
    {
        ...
        {
            if (none_of(resources.begin(), resources.end(), [&](const resourceCard &rc)
                        { return rc.get_type() == res; }))
            {
                return false;
            }
        }
    ...
    {
        if (count_if(development_cards.begin(), development_cards.end(), [](developeCard *c)
                     { return c->get_type() == "Knight"; }) >= 3)
        {
            add_const_score(2);
    ...
    {
        auto it = find_if(development_cards.begin(), development_cards.end(), [&](developeCard *c)
                          { return c->get_type() == card_type; });
    ...
    void player::takeResourceFromPlayer(player &player, const string &resource)
    {
        ...
        auto it = remove_if(player_resources.begin(), player_resources.end(), [&](resourceCard &rc)
                            { return rc.get_type() == resource; });
    ...
    {
        static random_device rd;              // random number generator
        static mt19937 gen(rd());             // seeder
        uniform_int_distribution<> dis(1, 6); // Uniform distribution for dice rolls (1 to 6)
        int roll1 = dis(gen);
        int roll2 = dis(gen);
            ...
            try
            {
                ...
                if (b.getTile(i).getvalue_roll() == roll)
                {
                    for (int j = 0; j < 6; j++)
                    {
                        try
                        {
                            const int type = vertexes[1][j];
                            const int id = vertexes[0][j];
